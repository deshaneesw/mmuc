...
muc <- function(y,a,m,cv,Data,conf.level=0.95,a.link="logit") {
  ...
  est<-function(g){
    h<-rep(0,ng);
    pi<-inv.link(as.vector(cbind(1,C)%*%g[1:(1+nc)]),link=a.link)
    beta<-g[(2+nc):(1+nc+(2+nc)*nmed)]
    theta_1<-g[1+nc+(2+nc)*nmed+1]
    theta_2<-g[(nc+(2+nc)*nmed+3):(nc+(2+nc)*nmed+2+nmed)]
    h[1:(1+nc)]<-t(cbind(1,C))%*%(Dt[,a]-pi)
    h[(2+nc):(1+nc+(2+nc)*nmed)]<-as.vector(t(cbind(1,Dt[,a],C))%*%as.matrix(Dt[,m]-cbind(1,Dt[,a],C)%*%matrix(beta,ncol=nmed)))
    h[1+nc+(2+nc)*nmed+1]<-t(Dt[,a]-pi)%*%(Dt[,y]-Dt[,a]*theta_1-as.matrix(Dt[,m])%*%theta_2)
    h[(2+nc+(2+nc)*nmed+1):(2+nc+(2+nc)*nmed+nmed)]<- t(Dt[,a]-pi)%*%as.matrix((Dt[,m]-cbind(1,Dt[,a],C)%*%matrix(beta,ncol=nmed))*as.vector(Dt[,y]-as.matrix(Dt[,m])%*%theta_2))
    h
  }
  ...
  est.fun<-function(g){
    ...
    f<-cbind(1,C)*(Dt[,a]-pi)
    for (i in 1:nmed) {
      f<-cbind(f,cbind(1,Dt[,a],C)*as.vector(Dt[,m[i]]-cbind(1,Dt[,a],C)%*%(matrix(beta,ncol=nmed)[,i])))
    }
    f<-cbind(f,(Dt[,a]-pi)*(Dt[,y]-Dt[,a]*theta_1-as.matrix(Dt[,m])%*%theta_2))
    for (i in 1:nmed) {
      f<-cbind(f,(Dt[,a]-pi)*((Dt[,m[i]]-cbind(1,Dt[,a],C)%*%(matrix(beta,ncol=nmed)[,i]))*(Dt[,y]-as.matrix(Dt[,m])%*%theta_2)))
    }
    f
  }
  ...
  nie.m<-sol$par[seq(3+nc,(1+nc+(2+nc)*nmed),2+nc)]*sol$par[seq(3+nc+(2+nc)*nmed,1+nc+(2+nc)*nmed+1+nmed,1)]
  nie<-sum(nie.m)
  ...
  for (i in seq(3+nc+(2+nc)*nmed,1+nc+(2+nc)*nmed+1+nmed,1)) {
    h<-append(h,c(0,sol$par[i],rep(0,nc)))
  }
  h<-append(h,0)
  for (i in seq(3+nc,(1+nc+(2+nc)*nmed),2+nc)) {
